<template>
  ...
  <div class="noResults" v-if="noResults">
    ...
    <div v-for="result in results" :key="result._id">
      <input type="checkbox" :id="result._id" :value="result._id" v-model="checkedBoxes">
      <label :for="result._id">{{result._source.roman}} - {{result._source.definition}}</label>
  </div>
  {{checkedBoxes}}
</template>
...
export default {
  ...
  methods: {
    queryMorpheme() {
      ...
      stretchy.searchTerm(this.query)
        .then((response) => {
